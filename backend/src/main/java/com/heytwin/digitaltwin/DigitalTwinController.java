...
public class DigitalTwinController {
    ...
    public ResponseEntity<ApiResponse<DigitalTwinSummary>> summary(Principal principal) {
        ...
        return ResponseEntity.ok(ApiResponse.<DigitalTwinSummary>builder().status(200).path("/api/digital-twin/summary").data(summary).build());
    }

    private UUID resolveStudentId(Principal principal) {
        User user = userRepository.findByEmailIgnoreCase(principal.getName()).orElseThrow(() -> new IllegalArgumentException("User not found"));
        return user.getId();
    }

    @java.lang.SuppressWarnings("all")
    
    public DigitalTwinController(final DigitalTwinService digitalTwinService, final UserRepository userRepository) {
