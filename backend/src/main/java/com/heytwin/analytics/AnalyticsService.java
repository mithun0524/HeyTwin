...
public class AnalyticsService {
    ...
    public Map<String, Object> learningCurve(User student) {
        ...
        List<Map<String, Object>> payload = sessions.stream().limit(10).map(session -> {
            ...
            double avgTime = responses.stream().mapToInt(response -> response.getTimeTakenSec() != null ? response.getTimeTakenSec() : 45).average().orElse(45.0);
    ...
    public Map<String, Object> topicPie(User student) {
        ...
        List<Map<String, Object>> topics = profiles.stream().map(profile -> {
    ...
    public Map<String, Object> spacedReminders(User student) {
        List<Map<String, Object>> reminders = profileRepository.findByStudent(student).stream().filter(profile -> profile.getForgettingScore() < 0.5).map(profile -> {
    ...
    public Map<String, Object> achievements(User student) {
        ...
        List<Map<String, Object>> earnedPayload = earned.stream().filter(StudentAchievement::getAwarded).map(item -> {
        ...
        List<Map<String, Object>> progress = earned.stream().filter(item -> !item.getAwarded()).map(item -> {
    ...
    public Map<String, Object> estimatedScore(User student) {
        ...
        return Map.of("estimatedScore", Math.round(estimated), "confidence", summary.getConsistency(), "inputs", Map.of("mastery", summary.getAccuracy(), "consistency", summary.getConsistency(), "recentAverage", summary.getDifficultyScore()));
    }

    @java.lang.SuppressWarnings("all")
    
    public AnalyticsService(final PracticeSessionRepository practiceSessionRepository, final ResponseRepository responseRepository, final StudentTopicProfileRepository profileRepository, final StudentAchievementRepository achievementRepository, final DigitalTwinService digitalTwinService) {
